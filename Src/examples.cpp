...
void fitAT2025wpp() {
	...
	//min and max parameters, which defind the region to find minimum. also max parameters are used for normalization of units
	double minParameters[Nparams] = { 0.6 * rmax, 0.000001, 1, 0.05, 0.5 * speed_of_light };
	double maxParameters[Nparams] = { timeMoment * 0.8 * speed_of_light, 0.5, 2000, 0.2, 0.8 * speed_of_light };
	//starting point of optimization and normalization
	double vector[Nparams] = { rmax, sigma, electronConcentration, widthFraction, v };
	for (int i = 0; i < Nparams; ++i) {
		vector[i] = vector[i] / maxParameters[i];
	...
	electronDistribution->writeDistribution("distributionAT2025.dat", 1000, me_c2, 1E9 * me_c2);
	...
	//number of points per axis in gridEnumOptimizer
	int Npoints[Nparams] = { 4,20,20,4,4 };
	//number of iterations in gradient descent optimizer
	int Niterations = 4;
	//energies of electrons wich will be used for evaluatig radiation
	double Emin = me_c2;
	...
	double* uncertainties = combinedOptimizer->evaluateUncertainties(vector, optPar);
	...
	double kevFluxDisk1 = evaluator2->evaluateTotalFluxInEnergyRange(0.3 * keV, 10 * keV, 400, source3);

	double kevFluxDisk2 = evaluator2->evaluateTotalFluxInEnergyRange(0.3 * keV, 30 * keV, 400, source3);

	double kevFluxDisk3 = evaluator2->evaluateTotalFluxInEnergyRange(20 * keV, 200 * keV, 400, source3);
	...
	printf("keV flux disk 0.3-10 keV= %g, luminosity = %g\n", kevFluxDisk1, kevFluxDisk1 * 4 * pi * distance * distance);
	printLog("keV flux disk 0.3-10 keV= %g, luminosity = %g\n", kevFluxDisk1, kevFluxDisk1 * 4 * pi * distance * distance);

	printf("keV flux disk 0.3-30 keV= %g, luminosity = %g\n", kevFluxDisk2, kevFluxDisk2 * 4 * pi * distance * distance);
	printLog("keV flux disk 0.3-30 keV= %g, luminosity = %g\n", kevFluxDisk2, kevFluxDisk2 * 4 * pi * distance * distance);

	printf("keV flux disk 20-200 keV= %g, luminosity = %g\n", kevFluxDisk3, kevFluxDisk3 * 4 * pi * distance * distance);
	printLog("keV flux disk 20-200 keV= %g, luminosity = %g\n", kevFluxDisk3, kevFluxDisk3 * 4 * pi * distance * distance);
...
void evaluateV4641() {
	double energy1[2] = {0.816, 1.217};
	double observedFlux1[2] = {75, 87};
	double observedError1[2] = {26, 13};
	//int Nenergy1 = readRadiationFromFile(energy1, observedFlux1, observedError1, "./examples_data/css_data/coppejans69.txt");
	int Nenergy1 = 2;
	for (int i = 0; i < Nenergy1; ++i) {
		energy1[i] = energy1[i] * hplank * 1E9;
	...
	//distance to source
	const double distance = 6.2 * 1E3 * parsec;

	//initial parameters of source
	double electronConcentration = 17 * 5;
	double angle = (10.0 / 60.0) * pi / 180.0;//10 arcmin
	double rmax = angle*distance;
	//rmax = 1.0;
	double B = 2E-5;
	double widthFraction = 0.1;
	double v = 0.75 * speed_of_light;
	...
	//min and max parameters, which defind the region to find minimum. also max parameters are used for normalization of units
	double minParameters[Nparams] = { 0.6 * rmax, 0.000001, 1, 0.05, 0.5 * speed_of_light };
	double maxParameters[Nparams] = { rmax, 0.1, 500, 0.2, 0.8 * speed_of_light };
	//starting point of optimization and normalization
	double vector[Nparams] = { rmax, sigma, electronConcentration, widthFraction, v };
	for (int i = 0; i < Nparams; ++i) {
		vector[i] = vector[i] / maxParameters[i];
...
void fitCSS161010_2() {
	...
	//min and max parameters, which defind the region to find minimum. also max parameters are used for normalization of units
	double minParameters[Nparams] = { 0.6 * rmax, 0.000001, 1, 0.05, 0.5 * speed_of_light};
	double maxParameters[Nparams] = { timeMoment * 0.8 * speed_of_light, 0.1, 200, 0.2, 0.8 * speed_of_light};
	//starting point of optimization and normalization
	double vector[Nparams] = { rmax, sigma, electronConcentration, widthFraction, v};
	for (int i = 0; i < Nparams; ++i) {
		vector[i] = vector[i] / maxParameters[i];
...
void testBigSource() {
	...
	RectangularSourceWithSynchAndComptCutoffFromRight* source = new RectangularSourceWithSynchAndComptCutoffFromRight(Nx, xgrid, Ny, Nz, electrons1, B, Btheta, Bphi, concentrationArray, 0, size, 0, pi * size, distance, u, u, photonTotalEnergyDensity);

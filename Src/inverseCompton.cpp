...
InverseComptonEvaluator::InverseComptonEvaluator(int Ne, int Nmu, int Nphi, double Emin, double Emax, double Ephmin, double Ephmax, PhotonDistribution* photonDistribution, ComptonSolverType solverType) : RadiationEvaluator(Ne, Emin, Emax){
...
void InverseComptonEvaluator::outputDifferentialFlux(const char* fileName) {
	...
	double photonFinalEnergy = 0.0001 * massElectron * speed_of_light2;
	double photonFinalTheta = 0;
	double photonFinalPhi = 0;

	double electronInitialEnergy = 1.00001 * massElectron * speed_of_light2;
	double electronInitialTheta = 0;
	double electronInitialPhi = 0;

	double photonInitialTheta = pi;
	...
	//for (int i = 0; i < Nph; ++i) {
		...
		fprintf(outFile, "%15.10g %15.10g  %15.10g\n", photonInitialTheta, evaluateDifferentialFlux(photonFinalEnergy, photonFinalTheta, photonFinalPhi, electronInitialEnergy, electronInitialTheta, electronInitialPhi, photonInitialTheta, photonInitialPhi), my_dcosTheta[i]);
...
double InverseComptonEvaluator::evaluateComptonFluxThomsonIsotropic(const double& photonFinalEnergy, PhotonDistribution* photonDistribution, MassiveParticleDistribution* electronDistribution, const double& volume, const double& distance) {
...
double InverseComptonEvaluator::evaluateComptonFluxJonesIsotropic(const double& photonFinalEnergy, PhotonDistribution* photonDistribution, MassiveParticleDistribution* electronDistribution, const double& volume, const double& distance) {
		...
		for (int k = 0; k < my_Ne; ++k) {
			...
			if (electronInitialGamma > photonFinalEnergy / (m_c2)) {
				...
				if (q <= 1.0) {
					...
					//divide by energy to get number of photons
					//I += electronDistribution->distribution(electronInitialEnergy) * volume * sigma * photonDistribution->distribution(photonInitialEnergy)  * speed_of_light * electronInitialBeta * delectronEnergy * dphotonInitialEnergy / (massElectron * speed_of_light2);
					double electronDist = isotropicElectronDistribution->distribution(electronInitialEnergy);
					I += photonFinalEnergy * 4 * pi * electronDist * volume * sigma * isotropicPhotonDistribution->distribution(photonInitialEnergy) * speed_of_light * electronInitialBeta * delectronEnergy * dphotonInitialEnergy / m_c2;
					if (I < 0) {
						omp_set_lock(&my_lock);
						printf("evaluateComptonFluxJonesIsotropic\n");
						printLog("evaluateComptonFluxJonesIsotropic\n");
						...
						printf("electronInitialEnergy = %g\n", electronInitialEnergy);
						printLog("electronInitialEnergy = %g\n", electronInitialEnergy);
						printf("electron distribution = %g\n", isotropicElectronDistribution->distribution(electronInitialEnergy));
						printLog("electron distribution = %g\n", isotropicElectronDistribution->distribution(electronInitialEnergy));
						printf("volume = %g\n", volume);
						printLog("volume = %g\n", volume);
						printf("photon initial energy = %g\n", photonInitialEnergy);
						printLog("photon initial energy = %g\n", photonInitialEnergy);
						printf("photon distrinution = %g\n", isotropicPhotonDistribution->distribution(photonInitialEnergy));
...
double InverseComptonEvaluator::evaluateComptonFluxKleinNishinaIsotropic(const double& photonFinalEnergy, PhotonDistribution* photonDistribution, MassiveParticleDistribution* electronDistribution, const double& volume, const double& distance) {
	MassiveParticleIsotropicDistribution* isotropicElectronDistribution = dynamic_cast<MassiveParticleIsotropicDistribution*>(electronDistribution);
	if (isotropicElectronDistribution == NULL) {
			...
			double phi_e = 0;
				...
				for (int iphiph = 0; iphiph < my_Nphi; ++iphiph) {
					...
					for (int imuph = 0; imuph < my_Nmu; ++imuph) {
						...
						double photonsN = isotropicPhotonDistribution->distribution(photonInitialEnergy);
...
double InverseComptonEvaluator::evaluateComptonFluxKleinNishinaIsotropic2(const double& photonFinalEnergy, PhotonDistribution* photonDistribution, MassiveParticleDistribution* electronDistribution, const double& volume, const double& distance) {
	MassiveParticleIsotropicDistribution* isotropicElectronDistribution = dynamic_cast<MassiveParticleIsotropicDistribution*>(electronDistribution);
	if (isotropicElectronDistribution == NULL) {
		printf("Isotropic Klein Nishina solver works only with isotropic distribution\n");
		printLog("Isotropic Klein Nishina solver works only with isotropic distribution\n");
	...
	for (int k = 0; k < my_Ne; ++k) {
		...
		for (int imue = 0; imue < my_Nmu; ++imue) {
			...
			for (int iphie = 0; iphie < my_Nphi; ++iphie) {
				double phi_e = 2 * pi * (iphie + 0.5) / my_Nphi;
				double dphi_e = 2*pi / my_Nphi;
				double electronDist = electronDistribution->distribution(electronInitialEnergy, cos(theta_e), phi_e);
			//double phi_e = 0;
			//rotation
			double photonFinalThetaRotated;
			double photonFinalPhiRotated;
...
//uses lab rotated integration and formula 4.31
double InverseComptonEvaluator::evaluateComptonFluxKleinNishinaAnisotropic2(const double& photonFinalEnergy, const double& photonFinalTheta, const double& photonFinalPhi, PhotonDistribution* photonDistribution, MassiveParticleDistribution* electronDistribution, const double& volume, const double& distance) {
	double m = electronDistribution->getMass();
	double m_c2 = m * speed_of_light2;
	double r2 = sqr(electron_charge * electron_charge / m_c2);
...
double InverseComptonEvaluator::evaluateComptonFluxKleinNishinaAnisotropic3(const double& photonFinalEnergy, const double& photonFinalTheta, const double& photonFinalPhi, PhotonDistribution* photonDistribution, MassiveParticleDistribution* electronDistribution, const double& volume, const double& distance) {
	double m = electronDistribution->getMass();
	double m_c2 = m * speed_of_light2;
	double r2 = sqr(electron_charge * electron_charge / m_c2);
...
double InverseComptonEvaluator::evaluateComptonFluxKleinNishinaAnisotropic(const double& photonFinalEnergy, PhotonDistribution* photonDistribution, MassiveParticleDistribution* electronDistribution, const double& volume, const double& distance) {
	...
	return evaluateComptonFluxKleinNishinaAnisotropic2(photonFinalEnergy, 0, 0, photonDistribution, electronDistribution, volume, distance);
}

double InverseComptonEvaluator::evaluateFluxFromFunction(const double& photonFinalEnergy, MassiveParticleDistribution* electronDistribution, const double& volume, const double& distance, PhotonDistribution* photonDistribution) {
	if (my_solverType == ComptonSolverType::ISOTROPIC_THOMSON) {
		return evaluateComptonFluxThomsonIsotropic(photonFinalEnergy, photonDistribution, electronDistribution, volume, distance);
	}
	...
	else if (my_solverType == ComptonSolverType::ANISOTROPIC_KLEIN_NISHINA2) {
		return evaluateComptonFluxKleinNishinaAnisotropic2(photonFinalEnergy, 0, 0, photonDistribution, electronDistribution, volume, distance);
	}
	else if (my_solverType == ComptonSolverType::ANISOTROPIC_KLEIN_NISHINA3) {
		return evaluateComptonFluxKleinNishinaAnisotropic3(photonFinalEnergy, 0, 0, photonDistribution, electronDistribution, volume, distance);
	}
	else {
		...
		for (int iz = 0; iz < Nz; ++iz) {
			for (int iphi = 0; iphi < Nphi; ++iphi) {
				if (solverType == ComptonSolverType::ANISOTROPIC_KLEIN_NISHINA) {
					...
					result += evaluateComptonFluxKleinNishinaAnisotropic2(photonFinalEnergy, photonFinalTheta, photonFinalPhi, photonDistribution, source->getParticleDistribution(irho, iz, iphi), source->getVolume(irho, iz, iphi), source->getDistance());
				}
				else if (solverType == ComptonSolverType::ANISOTROPIC_KLEIN_NISHINA3) {
					result += evaluateComptonFluxKleinNishinaAnisotropic3(photonFinalEnergy, photonFinalTheta, photonFinalPhi, photonDistribution, source->getParticleDistribution(irho, iz, iphi), source->getVolume(irho, iz, iphi), source->getDistance());
				}
				else {
...
PhotonDistribution* InverseComptonEvaluatorWithSource::getPhotonDistribution(const double& rho, const double& z, const double& phi) {
	my_photonDistribution->resetConcentration(my_defaultPhotonConcentration);

...
def plot_W50_EFE_highenergy3(filename1, filename2, filename3, name, factor = 1.0):
    ...
    for i in range(N3):
        ...
        lhaasomodel[1,i] = radiation3[1,i] + 0.35*radiation2[1,i] + radiation1[1,i]

    hessmodel = np.zeros([2,N3])
    for i in range(N3):
        hessmodel[0,i] = radiation3[0,i]
        hessmodel[1,i] = 0.25*radiation2[1,i] + radiation1[1,i]
    ...
    plt.plot(radiation3[0], hessmodel[1], 'pink', linewidth=2, label='sum jet + thick from front')
    ...
    plt.errorbar(hess[0,:], hess[1, :], yerr = [hess[3, :], hess[2, :]], xerr = [hess[5, :], hess[4, :]], uplims = hessLimits, ecolor='purple', elinewidth=1.5, linewidth=0, capsize=2.5, capthick=1.5, label = "H.E.S.S.")

...
def plot_bremsstrahlung():
    ...
    for i in range(N):
        ...
        for j in range(Nrho):
            radiation[2*j + 1,i] = float(s[2*j+1])*1E-26
            radiation[2*j + 2,i] = float(s[2*j+2])*1E-26
        radiation[2*Nrho+1, i] = float(s[2*Nrho+1])*1E-26
    ...
    ax.set_ylabel(r'$F(\nu)~erg~cm^{-2} Hz^{-1} s^{-1}$', fontsize=40,fontweight='bold')
    ...
    plt.plot(radiation[0], radiation[2*Nrho+1],'r', linewidth = 4)

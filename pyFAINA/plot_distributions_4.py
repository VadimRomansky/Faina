...
def plot_distributions_4():
   ...
   #         a = 4*q*q*q*q*B*B*Ee[k,i]*mp*c*c*l/(9*(me**4)*(c**7)*u)
    ...
    ax.set_ylabel('f$_p$(p/m$_p$ c) (p/m$_p$ c)$^4$', fontsize=40,fontweight='bold')
    ax.set_yscale("log")
    ax.set_xscale("log")
    ax.set_xlim([0.05, 20])
    ax.set_ylim([0.001, 1])
    ...
    # ax.tick_params(axis='both', which='minor', labelsize=8)
    #ax.set_xlabel('$E_{kin}/m_p c^2$', fontsize=40, fontweight='bold')
    ax.set_xlabel('p/m$_e$ c', fontsize=40,fontweight='bold')
    #ax.set_ylabel('$f(E) E^2$', fontsize=40, fontweight='bold')
    ax.set_ylabel('f$_e$(p/m$_e$ c) (p/m$_e$ c)$^4$', fontsize=40,fontweight='bold')
    ax.set_yscale("log")
    ax.set_xscale("log")
    ax.set_xlim([0.1, 1E3])
    ax.set_ylim([1E-4, 10])

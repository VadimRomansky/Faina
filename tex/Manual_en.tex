%\documentclass[a4paper,12pt]{report}
\documentclass[a4paper,12pt]{extreport}


\input{packages}		% Подключаемые пакеты
\input{styles}			% Пользовательские стили

% journals

\def\apj{ApJ}
\def\apjl{ApJ Lett.}
\def\mnras{MNRAS}
\def\nat{Nature}
\def\physrevB{Phys. Rev. B}
\def\prd{Phys. Rev. D}
\def\prl{Phys. Rev. Lett.}
\def\pre{Phys. Rev. E}
\def\araa{Ann. Rev. Astron. Astrophys.}                % "Ann. Rev. Astron. Astrophys."
\def\aap{Astron. Astrophys.}                  % "Astron. Astrophys."
\def\aaps{Astron. Astrophys. Suppl. Ser.}                 % "Astron. Astrophys. Suppl. Ser."
\def\aj{Astron. J.}                      % "Astron. J."
\def\apjs{Astrophys. J. Suppl. Ser.}                  % "Astrophys. J. Suppl. Ser."
\def\pasp{Publ. Astron. Soc. Pac.}                  % "Publ. Astron. Soc. Pac."
\def\apjl{ApJ Lett.}                   % letter at ApJ
\def\pasj{PASJ}
\def\apss{Astroph. Space Sci.}
\def\aplett{Astroph. Lett.}
\def\ssr{Space Sci. Rev.}
\def\jcap{J. Cosmol. Astropart. Phys.}
\def\apspr{Sov. Sci. Rev. Sect. E}
\def\nar{New Astron. Rev.}
\def\aapr{ Astron. Astrophys. Rev.}

\begin{document}
	
	\input{names_en}			% Переопределение именований
	
	\input{title_en}			% Титульный лист
	\input{contents_en}		% Оглавление
	\input{introduction_en}	% Введение
	\input{part1_en}			% Глава 1. Источники
	\input{part2_en}            % Глава 2. Расчет излучения
	\input{part3_en}			% Глава 3. Оптимизация параметров
	\input{part4_en}			% Глава 4. Формулы расчета излучения
	\input{references}		% Список литературы
	%\input{appendix}     % Приложения
	
\end{document}
